...
function signEvent(event, privateKeyHex) {
  ...
  const signatureBytes = crypto.createHmac('sha256', Buffer.from(privateKeyHex, 'hex'))
    .update(event.id)
    .digest();
...
async function main() {
  ...
  const message = await prompt(`\nEnter message (press Enter for default): `) || defaultMessage;
  ...
  for (const relay of DEFAULT_RELAYS) {
    try {
      ...
      console.error(`${LOG_PREFIX} Failed to publish to ${relay}:`, error.message);
